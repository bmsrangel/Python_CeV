...
class clear_and_catch_warnings(warnings.catch_warnings):
    ...
    the warnings filters.  This context manager accepts a sequence of `classes`
    as a keyword argument to its constructor and:

    * stores and removes any ``__warningregistry__`` entries in given `classes`
      on entry;
    * resets ``__warningregistry__`` to its previous state on exit.
    ...
    modules : sequence, optional
        Sequence of classes for which to reset warnings registry on entry and
        restore on exit. To work correctly, all 'ignore' filters should
        filter by one of these classes.

    Examples
    --------
    >>> import warnings
    >>> with clear_and_catch_warnings(classes=[np.core.fromnumeric]):
    ...     warnings.simplefilter('always')
...
class suppress_warnings(object):
    ...
    def filter(self, category=Warning, message="", module=None):
        ...
        module : module, optional
            ...
            it unreliable for external classes.
