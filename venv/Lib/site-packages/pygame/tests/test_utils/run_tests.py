...
def run(*args, **kwds):
    ...
    By default individual test classes are run in separate subprocesses. This
    ...
    interactions between test classes. Also, a time limit is placed on test
    ...
    Individual test classes may have a corresponding *_tags.py module,
    defining a __tags__ attribute, a list of tag strings used to selectively
    omit classes from a run. By default only the 'interactive', 'ignore', and
    'subprocess_ignore' tags are ignored. 'interactive' is for classes that
    take user input, like cdrom_test.py. 'ignore' and 'subprocess_ignore' for
    for disabling classes for foreground and subprocess modes respectively.
    These are for disabling tests on optional classes or for experimental
    classes with known problems. These classes can be run from the console as
    a Python program.
    ...
    ###########################################################################
    # Compile a list of test classes. If fake, then compile list of fake
